...
bool MainWindow::isValidIntersection(QPointF intersection, QPointF P1, QPointF P2) {
    double epsilon = 0.00001;
    return intersection.x() >= qMin(P1.x(), P2.x()) - epsilon &&
           intersection.x() <= qMax(P1.x(), P2.x()) + epsilon &&
           intersection.y() >= qMin(P1.y(), P2.y()) - epsilon &&
           intersection.y() <= qMax(P1.y(), P2.y()) + epsilon;
}

bool MainWindow::clipMidpoint(QPointF P1, QPointF P2) {
    double x1 = P1.x(), y1 = P1.y();
    double x2 = P2.x(), y2 = P2.y();

    // Проверяем, полностью ли отрезок находится вне окна
    if ((x1 < Xmin && x2 < Xmin) || (x1 > Xmax && x2 > Xmax) ||
        (y1 < Ymin && y2 < Ymin) || (y1 > Ymax && y2 > Ymax)) {
        return false;
    }

    // Используем алгоритм средней точки для проверки вхождения отрезка в окно
    double dx = x2 - x1;
    double dy = y2 - y1;
    double dxAbs = qAbs(dx);
    double dyAbs = qAbs(dy);

    int steps = qMax(dxAbs, dyAbs);
    ...
    for (int step = 0; step <= steps; ++step) {
        // Проверяем, находится ли текущая точка в окне
        if (x >= Xmin && x <= Xmax && y >= Ymin && y <= Ymax) {
            return true; // Точка внутри окна, отрезок не полностью отсечен
        }
    ...
    return false; // Все точки отрезка находятся вне окна
}
